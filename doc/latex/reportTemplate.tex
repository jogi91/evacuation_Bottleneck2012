...
Because of this, we cannot answer our question, wether a high panic level (which means a high value for \(v_0\))
with few people
...
In the simulation, two ways were defined to react to a full boat, as described in Section~\ref{sub:exitremove}.  First, there is an instantanous update on the whole ship.  This is a model for an
...
behaviour is realistic in a case when the following conditions are met:
...
However, the effect of the pushback on the evacuation time is clearly visible in Figure~\ref{evactimescomparemany} and \ref{evactimescomparefew}.
...
In the runs with instantanous propagation, one agent was stuck for \(1.3\frac{m}{s}\), two for \(1.4\frac{m}{s}\) and two for \(1.5\frac{m}{s}\).

Agents, that are informed instantanously can adapt their way earlier and reduce the overall
...
\begin{figure}[h]
	...
	\caption{Comparison of evacuation times with 700 agents and varying \(v_0\) with instantanous or radial update. 	}
...
\begin{figure}[h]
	...
	\caption{Comparison of evacuation times with 350 agents and varying \(v_0\) with instantanous or radial update. 	}
...
Eventhough we could not answer all our questions, we got some new
